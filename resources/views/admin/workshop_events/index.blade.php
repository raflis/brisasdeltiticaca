    ...
    <div class="container-fluid">
        <div class="row layout-header">
            <div class="col-sm-12 header-content">
                <h1>
                    <i class="fas fa-bullhorn fa-xs text-white2"></i> Talleres
                </h1>
                <span class="subtitle">
            ...
            <div class="col-sm-12">
                <div class="card shadow">
                    <div class="card-header">
                        ...
                        <div>
                            <a class="btn btn-success" href="{{ route('workshop_events.create') }}">
                                <span class="icon">
                ...
                <div class="tablaTotal">
                    <table class="table tableD">
                        ...
                        <tbody>
                            @foreach ($workshop_events as $item)
                            <tr>
                                <td>{{ $workshop_events->firstItem() + $loop->index }}</td>
                                <td>{{ $item->name }}</td>
                                <td>{{ $item->workshop->name }}</td>
                                <td>{{ \Carbon\Carbon::parse($item->event_date)->format('d/m/Y') }}</td>
                                <td>
                                    <div style="display: inline-flex">
                                        <a class="btn btn-primary text-white btn-sm mr-1" href="{{ route('workshop_events.edit', $item->id) }}">
                ...
                <div class="paginacionTotal d-flex justify-content-end">
                    {{ $workshop_events->withQueryString()->render() }}
                </div>
            </div>
        </div>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-footer justify-content-center">
            ...
            {!! Form::open(['route' => ['workshop_events.destroy', ''], 'method' => 'DELETE', 'id' => 'form-eliminar']) !!}
...
$('document').ready(function(){
    $('.btn-eliminar').click(function(e){
        ...
        var base = '{{ route('workshop_events.destroy', '') }}';
        var url = base + '/' +id;
